const CACHE="lenton-pwa-pwa-2f4df8282f-android-305-1daddfa1ebb0";
const SHELL=["./","./index.html?v=pwa-2f4df8282f-android-305-1daddfa1ebb0","./styles.css?v=pwa-2f4df8282f-android-305-1daddfa1ebb0","./app.js?v=pwa-2f4df8282f-android-305-1daddfa1ebb0","./manifest.webmanifest?v=pwa-2f4df8282f-android-305-1daddfa1ebb0","./generated/android-spec.js?v=pwa-2f4df8282f-android-305-1daddfa1ebb0","./changelog.json?v=pwa-2f4df8282f-android-305-1daddfa1ebb0","./icon-192.png?v=pwa-2f4df8282f-android-305-1daddfa1ebb0","./icon-512.png?v=pwa-2f4df8282f-android-305-1daddfa1ebb0","./icon-1024.png?v=pwa-2f4df8282f-android-305-1daddfa1ebb0"];
...
self.addEventListener("fetch",event=>{
  ...
  if(req.mode==="navigate"){
    event.respondWith(networkFirst(req,"./index.html?v=pwa-2f4df8282f-android-305-1daddfa1ebb0"));
    return;
  }
  if(
...
async function rootPushBadge(data){
  ...
  if(type==="mention"&&detail?.status?.visibility==="direct")kind="dm";
  if(match){
  ...
  return {match,total,kind,detail};
}
...
self.addEventListener("push",event=>{
  ...
  event.waitUntil((async()=>{
    ...
    const icon=badgeInfo.detail?.account?.avatar_static||badgeInfo.detail?.account?.avatar||data.icon||n.icon||badgeInfo.match?.avatar||"./icon-1024.png?v=pwa-2f4df8282f-android-305-1daddfa1ebb0";
    const target=n.navigate||data.url||(notificationId?("./?notification_id="+encodeURIComponent(notificationId)):"./?view=notifications");
    const options={body,icon,badge:"./icon-1024.png?v=pwa-2f4df8282f-android-305-1daddfa1ebb0",tag:notificationId||"lenton-notification",renotify:true,data:{url:target,accountKey,notificationId,icon,kind},silent:false};
    await Promise.all([
